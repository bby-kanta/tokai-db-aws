...
#### 一番好きなメンバー
ユーザーはマイページに一番好きなメンバーを登録することができます。
そのユーザーが誰を推しているのかが一目で分かります。
...
### 関連動画を表示する機能  
例えば前編・後編があったりする動画に大きな効果を発揮する機能です。
現在見ている記事の一番上に関連動画が優先して表示されるようになっています。  
...
また見所にすぐ飛ぶことができる「時間指定のハイパーリンク」があるのでYouTubeと同じような感覚で見れると思います。
その際クリックすると動画がある場所まで自動スクロールします。
...
## Ruby on rails 6  　　
WebPackerが標準で備わっているので、その勉強も行いました。
...
## Docker  
最初はHerokuにデプロイしただけだったんですがモダンな技術を使ってみたいという思いからDockerで構築し直しました。
